            ...
            <div class="row">
                    ...
                    <select class="popup pt-0 pb-0" id="mission_id" name="mission_id">
                        ...
                        @foreach ($missions as $mission)
                            <option value="{{ $mission->mission_id }}"
                                @if ($story) {{ $story->mission_id == $mission->mission_id ? 'selected' : '' }} @endif>
                ...
                <div class="col">
                    ...
                    <input type="text" class="popup" name="title"
                        value="@if ($story) {{ $story->title }} @endif"
                ...
                <div class="col removecal">
                    <p class="mb-1">Date</p>
                    <input type="datetime-local" step="1" name="published_at"
                        value="@if ($story) {{ $story->published_at }} @endif" class="popup">
                </div>
            </div>
            <p class="mb-1 mt-4">My Story</p>
            <textarea rows="5" placeholder="Enter your message" name="description" class="popup1">
@if ($story)
{{ $story->description }}
@endif
</textarea>
            ...
            <textarea rows="2" placeholder="Enter your url" name="url" class="popup1">
@if ($story_url)
@foreach ($story_url as $url)
{{ $url->path }}
@endforeach
@endif
</textarea>
                            ...
                            for (i = 0; i < filesAmount; i++) {
                                ...
                                reader.onload = function(event) {
                                    $($.parseHTML('<img>')).attr('src', event.target.result).appendTo(
                                        placeToInsertImagePreview).height(80).width(80).addClass('hj');
                                }
            ...
            <input type="file" name="media_name[]" id="gallery-photo-add" onchange="readURL(this);" multiple=""
                class="dnone">
            ...
            <div class="d-flex justify-content-between mt-3 mb-5">
                ...
                <div>
                    <button id="button" type="submit" name="add" value="draft"
                        class="scrollToTopButton col-example7">Save</button>
                    <button id="button2" type="submit" name="add" value="publish"
                        class="scrollTopButton col-example7" {{ $story ? '' : 'disabled' }}>Submit</button>
                </div>
            </div>
        </form>
    </div>
