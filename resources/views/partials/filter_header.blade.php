<form class="mb-0 border-sm-block mt13" id="selectSort3" action="{{ route('home.index') }}" method="PUT"
                        ...
                        <tr class="bs">
                            <td class="p-2">
                                <input type="text" value="" id="clickedButton" name="search"
                                    placeholder="Search mission..." class="form-control shadow-none m-2 border-0 fs18"
                                    onchange="showHide2()">
                            </td>
                            <td>
                                <nav class="navbar navbar-expand-sm bgw">
                                    <div class="container-fluid justify-content-end">
                                        <button class="navbar-toggler p-0 border-0" type="button"
                                            ...
                                            <img src="/storage/images/filter.png"
                                                class="input-group-text m-0 p-0 bgw border-0" id="basic-addon1"
                                                height="30px">
                                        </button>
                                    </div>
                                </nav>
                                        ...
                                        <div class="collapse navbar-collapse w-100" id="navbarSupportedContent2">
                                            <ul class="navbar-nav me-auto mb-2 mb-lg-0 fs-6">
                                                <li class="nav-item dropdown">
                                                    <select class="Rounded-Rectangle-9 w-100" name="country"
                                                        onchange="showHide2()">
                                                        <option value="none" hidden="">Country</option>
                                                        @foreach ($countries as $country)
                                                            <option value="{{ $country->name }}"
                                                                @if (request()->country) @if ($country->name == request()->country) 
                                                        {{ 'selected' }} @endif
                                                                @endif >{{ $country->name }}
                                                            </option>
                                                        @endforeach
                                                ...
                                                <li class="nav-item dropdown m25">
                                                    <a class="w-100 nav-link dropdown-toggle" href="#"
                                                    ...
                                                    <ul class="dropdown-menu p-0" aria-labelledby="navbarDropdown">
                                                        @foreach ($cities as $city)
                                                            <input type="checkbox" name="city[]"
                                                                value="{{ $city->name }}" onchange="showHide2()"
                                                                @if (request()->city) @foreach (request()->city as $item) 
                                                        @if ($city->name == $item) 
                                                        {{ 'checked' }} @endif
                                                                @endforeach
                                                        @endif> {{ $city->name }}<br>
                                                    ...
                                                    <ul class="dropdown-menu p-0" aria-labelledby="navbarDropdown">
                                                        @foreach ($themes as $theme)
                                                            <input type="checkbox" name="theme[]"
                                                                value="{{ $theme->title }}" onchange="showHide2()"
                                                                @if (request()->theme) @foreach (request()->theme as $item) 
                                                        @if ($theme->title == $item) 
                                                        {{ 'checked' }} @endif
                                                                @endforeach
                                                    ...
                                                    <ul class="dropdown-menu p-0" aria-labelledby="navbarDropdown">
                                                        @foreach ($skills as $skill)
                                                            <input type="checkbox" name="skill[]"
                                                                value="{{ $skill->skill_name }}" onchange="showHide()"
                                                                @if (request()->skill) @foreach (request()->skill as $item) 
                                    ...
                                    <ul class="w-100 navbar-nav me-auto mb-2 mb-lg-0">
                                        <li class="w-100 nav-item dropdown fs-6">
                                            ...
                                            <ul class="dropdown-menu p-0" aria-labelledby="navbarDropdown">
                                                @foreach ($cities as $city)
                                                    <input type="checkbox" name="city[]"
                                                        value="{{ $city->name }}" onchange="showHide()"
                                                        @if (request()->city) @foreach (request()->city as $item) 
                                                @if ($city->name == $item) 
                                                {{ 'checked' }} @endif
                                                        @endforeach
                                    ...
                                    <ul class="w-100 navbar-nav me-auto mb-2 mb-lg-0">
                                        <li class="w-100 nav-item dropdown fs-6">
                                            ...
                                            <ul class="dropdown-menu p-0" aria-labelledby="navbarDropdown">
                                                @foreach ($themes as $theme)
                                                    <input type="checkbox" name="theme[]"
                                                        value="{{ $theme->title }}" onchange="showHide()"
                                                        @if (request()->theme) @foreach (request()->theme as $item) 
                                                @if ($theme->title == $item) 
                                                {{ 'checked' }} @endif
                                                        @endforeach
        ...
        <div class="row row-cols-auto ps-4" id="text">
            @if (request()->skill)
                @foreach (request()->skill as $item)
                    <div class="p-1 six-txt pe-2 m-1">
                        <span>{{ $item }}</span>
                        <img src="storage/images/cancel.png" alt="" height="8px"
                            onclick="remove('{{ $item }}')" class="pe-auto">
                    </div>
                @endforeach
            @endif
            @if (request()->theme)
            ...
            @if (request()->country && request()->country != 'none')
                <div class="p-1 six-txt pe-2 m-1">
                    ...
                    <img src="storage/images/cancel.png" alt="" height="8px" onclick="removecountry()"
                        class="pe-auto">
                </div>
            @endif
                ...
                <div class="col-lg-7 col-mb-7 col-sm-7 col-0 df">
                    <div class="d-none d-lg-block">
                        <select class="Rounded-Rectangle-8" name="sort" onchange="showHide()">
                            ...
                            <option value="Newest" @if (request()->sort && request()->sort == 'Newest') {{ 'selected' }} @endif>
                                Newest</option>
                            <option value="Oldest" @if (request()->sort && request()->sort == 'Oldest') {{ 'selected' }} @endif>
                                Oldest</option>
                            <option value="Lowest available seats"
                                @if (request()->sort && request()->sort == 'Lowest available seats') {{ 'selected' }} @endif>Lowest available seats
                            </option>
                            <option value="Highest available seats"
                                @if (request()->sort && request()->sort == 'Highest available seats') {{ 'selected' }} @endif>Highest available seats
                            </option>
                            <option value="Sort by my favourite"
                                @if (request()->sort && request()->sort == 'Sort by my favourite') {{ 'selected' }} @endif>Sort by my favourite
                            </option>
                            <option value="sort by deadline"
                                @if (request()->sort && request()->sort == 'sort by deadline') {{ 'selected' }} @endif>sort by deadline
                            </option>
                        </select>
                        <a id="gridlink" class="ms-2" href="#"><i id="h1"
                                class="fa fa-th-large Ellipse-574 p-2 fs-5 text-secondary" aria-hidden="true"></i></a>
                        <a id="listlink" href="#"><i id="h2"
                                class="fa fa-list-ul p-2 fs-5 text-secondary" aria-hidden="true"></i></a>
                    </div>
                </div>
            </div>
        </div>
